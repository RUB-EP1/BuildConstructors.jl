function default_method_specs()
    return MethodSpec[
        ...
        MethodSpec(
            "Optim.Fminbox(LBFGS(); Minuit metric, ReverseDiff)",
            () -> (method = LBFGS(m = 10, scaleinvH0 = false), tolerance = 0.01, errordef = 0.5, autodiff = AutoReverseDiff()),
            true,
            :optim_minuit_lbfgs,
            "Bounded LBFGS with ReverseDiff gradients, descriptor-scaled preconditioning, and an EDM-style stopping proxy.",
        ),
        MethodSpec(
        ...
        MethodSpec(
            ...
            true,
            :optim_minuit_bfgs,
            "Bounded full-memory BFGS with ReverseDiff gradients, diagonal descriptor-scale initial inverse Hessian, and an EDM callback.",
        ),
